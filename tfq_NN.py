    ...
    def entangling_layer(self, circuit, n_qubits, l):
        for j in range(l):
            if n_qubits == 2:
                ...
                for i in range(n_qubits):
                    circuit.append(cirq.CZ(cirq.GridQubit(i + j * n_qubits, 0),
                                           cirq.GridQubit((i + 1) % n_qubits + j * n_qubits, 0)))
...
def main():
    ...
    quantum_layer = tfq.layers.PQC(model_circuit, observables)

    classical_dense_layer = tf.keras.layers.Dense(16, activation='relu')
    classical_output_layer = tf.keras.layers.Dense(1, activation='sigmoid')

    # Build the Keras model.
    model = tf.keras.Sequential([
        tf.keras.layers.Input(shape=(), dtype=tf.string),
        quantum_layer,
        classical_dense_layer,
        classical_output_layer
    ])
